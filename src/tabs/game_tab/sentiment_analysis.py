...
def get_overall_sentiment(sentiment_results):
    ...
    try:
        ...
        st.error(f"Error calculating overall sentiment: {str(e)}")
        return 0.0
...
def get_extreme_reviews(reviews_df, sentiment_results, count=10, sentiment_type="positive"):
    ...
    try:
        ...
        st.error(f"Error getting extreme reviews: {str(e)}")
        return pd.DataFrame()

# Use consistent path handling with other files
PARQUET_PATH = os.path.join("data", "cleaned_reviews_parquet") 