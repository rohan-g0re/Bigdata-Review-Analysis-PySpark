...
def filter_reviews_by_language(reviews_df, language):
    ...
    try:
        ...
        st.error(f"Error filtering reviews by language: {str(e)}")
        return pd.DataFrame()
...
def calculate_percentage(part, total):
    ...
    Returns:
        Float percentage or 0.0 if total is 0
    """
    try:
    ...
    if st.button("Search Game") and game_name:
        with st.spinner("Searching for game information..."):
            ...
            try:
                ...
                st.error(f"Error retrieving game information: {str(e)}")
                st.session_state.game_data = None
            finally:
                # Stop Spark session
                spark.stop()
    ...
    if st.session_state.game_data:
        ...
        try:
            ...
            with col2:
                ...
                if game_info.get('avg_playtime') is not None:
            ...
            with col3:
                ...
                early_access_count = game_info.get('early_access_count', 0)
                early_access_percent = calculate_percentage(early_access_count, game_info['total_reviews'])
                st.metric(
            ...
            st.error(f"Error displaying game information: {str(e)}")
